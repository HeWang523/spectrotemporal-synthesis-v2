...
% [b ,a] = butter(4, 3000 / (P.audio_sr / 2), 'high');
% wav_orig = filtfilt(b, a, wav_orig);

wav_orig = wav_orig ./ std(wav_orig);

% cochleogram filters
if P.overcomplete==0
    [audio_filts, audio_low_cutoff, cutoffs] = ...
        make_erb_cos_filters(duration_sec*P.audio_sr, P.audio_sr, ...
        P.n_filts, P.lo_freq_hz, P.audio_sr/2);

elseif P.overcomplete==1
    [audio_filts, audio_low_cutoff, freqs] = ...
...
f_out = P.lo_freq_hz*2.^(0:log2(P.audio_sr / (P.lo_freq_hz * 2))/P.n_filts:log2(P.audio_sr / (P.lo_freq_hz * 2)));
[pxx_fft,f_fft] = compute_FFT(wav_orig, P.audio_sr);
[~, W] = MarmosetExcitationPatternv1(f_fft, pxx_fft, cutoffs);
% load('C:\modelmatch\spectrotemporal-synthesis-v2-master\HeScripts\rox_filter_banks.mat');
% audio_filts = W;
...
for i = starting_iteration:P.n_iter+1
    ...
    if i > 1
        corr_temp = C.spectemp_mod_corr(end,1);
        var_temp = C.spectemp_mod_corr(end,2);
    else
        corr_temp = -1;
        var_temp = -1;
    end
    ...
    corr = C.spectemp_mod_corr(end,1);
    var = C.spectemp_mod_corr(end,2);

    if i > 1
        fprintf('\nLast iteration:\ncorr = %f\nvar = %f\n', corr, var);
    end

    % break if the moments of the spectrotemporal modulated synthesis sound almost matches the original sound
    % we can set the number of iterations to be a large number in this case

    % if 1 - corr_temp / corr <= 0.001 && 1 - var_temp / var <= 0.001
    %     break;
    % end
    ...
    close all;
    title_name = "";
    split_fname_without_extension = split(fname_without_extension, '_');
    for title_idx = 1:length(split_fname_without_extension)
        title_name = title_name + split_fname_without_extension{title_idx} + " ";
    end
    % Plot power specturm
    fig = figure('visible','off');
    [pxx_ori,~] = pspectrum(wav_orig ./ std(wav_orig), wav_sr, 'FrequencyLimits', [P.lo_freq_hz P.audio_sr/2],'FrequencyResolution',1024);
    [pxx_mm,f] = pspectrum(wav_synth ./ std(wav_synth), wav_sr, 'FrequencyLimits', [P.lo_freq_hz P.audio_sr/2],'FrequencyResolution',1024);

    semilogx(f/1000,pow2db(pxx_ori), 'r', LineWidth=2);
    hold on
    semilogx(f/1000,pow2db(pxx_mm),'b', LineWidth=2);
    legend('Original', 'Model Matched');
    ylim([-95, 20]);
    xlim([0, P.audio_sr / 2000]);
    box off;
    xlabel('Frequency (kHz)');
    ylabel('Power Spectrum (dB)');
    title([title_name 'Power Spectrum']);
    set(gcf,'color','w')
    hold off
    fname_full_path = [output_directory '/' fname_without_extension '_powerspectrum'];
    filename = fname_full_path + ".jpg";
    saveas(fig, filename);
    ...
    fig = figure('visible','off');
    f_out = P.lo_freq_hz*2.^(0:log2(P.audio_sr / (P.lo_freq_hz * 2))/P.n_filts:log2(P.audio_sr / (P.lo_freq_hz * 2)));
    [pxx_fft_ori,~] = compute_FFT(wav_orig, P.audio_sr);
    [pxx_fft_mm,f_fft] = compute_FFT(wav_synth, P.audio_sr);
    [ExP_ori, W] = MarmosetExcitationPatternv1(f_fft, pxx_fft_ori, f_out);
    [ExP_mm,  ~] = MarmosetExcitationPatternv1(f_fft, pxx_fft_mm, f_out);

    semilogx(f_out/1000, ExP_ori, 'r', LineWidth=2);
    hold on
    semilogx(f_out/1000, ExP_mm, 'b',LineWidth=2);
    legend('Original', 'Model Matched');
    xlabel('Central Frequency (kHz)');
    ylabel('Excitation Pattern (dB)');
    title([title_name 'Excitation Pattern']);
    ylim([-95, 20]);
    xlim([P.lo_freq_hz/1000, P.audio_sr / 2000]);
    hold off
    fname_full_path = [output_directory '/' fname_without_extension '_excitationpattern'];
    filename = fname_full_path + ".jpg";
    saveas(fig, filename);
